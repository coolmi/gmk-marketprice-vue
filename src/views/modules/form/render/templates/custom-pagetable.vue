<template>
  ...
  -->
  <div>
    <gmk-pagetable :ref="formName" v-if="Object.keys(formSchema).length>0" :form-name="formName" :form-schema="formSchema" :form-event="formEvent" :form-data="formData" :form-params="formParams" :state="state" @turnPage="trunPage" @search="search"></gmk-pagetable>
...
<script>
  ...
  export default {
    ...
    async created(){
      var _this = this;
      this.formEvent = events(this.formName, _this, 'pagetable');

      let res = await this.$http.get(this.$http.adornUrl('/form/schema/pagetable/' + this.formName));
    ...
    methods: {
      loadScript() {
        ...
        this.$nextTick(() => {
          this.setFormParam('page', 1);
          this.setFormParam('limit', 10);
          this.initData();
        })
      },
      initData() {
      ...
      trunPage(pageinfo) {
        this.setFormParam('page', pageinfo.pageIndex);
        this.setFormParam('limit', pageinfo.pageSize);
        this.initData();
      },
