<template>
  <el-form-item :label="formSchema.name" :prop="formSchema.key" :rules="rules"
                v-show="!hidden">
    <el-checkbox-group v-model="value" :disabled="disabled" @change="change">
      <el-checkbox v-for="(item, index) in options" :key="index" :label="item[optionValue]">{{ item[optionLabel] }}</el-checkbox>
    </el-checkbox-group>
  </el-form-item>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      value: {
        get: function () {
          if(this.formData != null && typeof this.formData != 'undefined' &&  this.formSchema.key in this.formData && this.formData[this.formSchema.key]) {
            return this.formData[this.formSchema.key].split(",");
          }
        ...
        set: function (newValue) {
          if(newValue && newValue.length > 0) {
            Vue.set(this.formData, this.formSchema.key, newValue.join(","));
          }
          else {
