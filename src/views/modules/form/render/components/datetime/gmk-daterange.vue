<template>
  <el-form-item :label="formSchema.name" :prop="formSchema.key" :rules="rules"
                v-show="!hidden">
    <el-date-picker style="width: 100%;" v-model="value" unlink-panels
                    :type="type" :value-format="valueFormat" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions"
      ...
      value: {
        get: function () {
          if(this.formData != null && typeof this.formData != 'undefined' &&  this.formSchema.key in this.formData) {
            if(this.formData[this.formSchema.key] != null && this.formData[this.formSchema.key] != '') {
              return this.formData[this.formSchema.key].split('至');
            }
          }
          if(this.formSchema.defaultval != null && this.formSchema.defaultval != '') {
        ...
        set: function (newValue) {
          if(newValue && newValue.length > 0) {
            Vue.set(this.formData, this.formSchema.key, newValue.join('至'));
          }
          else {
            Vue.set(this.formData, this.formSchema.key, '');
          }
        }
      },
      type: function() {
